...
public class GaleShapley {
    ...
    public static void main(String[] args) {
        int size = 10;   //Taille de chaque echantillon
        int seed = 10;  //Pour l'aleatoire
        ...
        while (couples.size() < size) {
            ...
            for (int i = 1; i < size; i++) {
                if (couples.containsKey(disposant.getId())) {
                    break; // Le disposant a déjà été apparié
                }
                int proposantId = disposant.getSouhait(i).getId();
                Proposants nouveauProposant = proposants.get(proposantId);
                if (nouveauProposant.getCouple() == -1) {
                ...
                } else if (disposant.proposantPrefere(proposant.getId(), nouveauProposant.getId())) {
    ...
    public static void main(String[] args) {
        ...
        PartialOrder[] womenPref = new PartialOrder({0, 2, 1});
        PartialOrder[] womenPref2 = new PartialOrder({2, 0, 1});
        PartialOrder[] womenPref3 = new PartialOrder({1, 2, 0}); // préférences des femmes
    ...
    public static void main(String[] args) {
        int size = 3;
        int[][] menPref = { {1, 2, 0}, {2, 1, 0}, {0, 2, 1} }; // préférences des hommes
        int[][] womenPref = {{0, 2, 1}, {2, 0, 1}, {1, 2, 0}};
        //PartialOrder[] womenPref = new PartialOrder({0, 2, 1});
        //PartialOrder[] womenPref2 = new PartialOrder({2, 0, 1});
        //PartialOrder[] womenPref3 = new PartialOrder({1, 2, 0}); // préférences des femmes
        int n = menPref.length; // nombre de personnes
